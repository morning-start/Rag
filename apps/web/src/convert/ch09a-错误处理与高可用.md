...
灾难恢复的核心指标是 RTO（Recovery Time Objective，恢复时间目标）和 RPO（Recovery Point Objective，恢复点目标）。针对 RAG 系统的各组件，建议的 RTO/RPO 如下：（来源: [RAG错误处理与高可用模式](reference/09-生产部署与运维/06-RAG错误处理与高可用模式/)）
