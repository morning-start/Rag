...
**本章技术选型**：我们将采用 **LangGraph StateGraph** 作为编排层，替代传统的 LangChain LCEL Chain。LangGraph 提供了更灵活的图结构、内置状态管理和条件路由能力，特别适合构建生产级 RAG 系统（来源: [03-LangGraph官方AgenticRAG教程.md](reference/04-工具链与环境/03-LangGraph官方AgenticRAG教程/)）。
...
本节内容已独立为[第5A章 LangGraph 完整 RAG 实战](ch05a-LangGraph完整RAG实战/)，包含完整的 LangGraph StateGraph 实现、代码示例和运行说明。

> 📖 继续阅读：[第5A章 LangGraph 完整 RAG 实战](ch05a-LangGraph完整RAG实战/)
