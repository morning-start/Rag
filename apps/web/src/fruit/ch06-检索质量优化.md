...
这种方法虽然增加 LLM 调用成本，但能显著提升上下文的信噪比。（来源: [RAG全链路优化方案](reference/06-检索策略与优化/01-RAG全链路优化方案体系/)）
...
**Lost in the Middle（LIM）效应**（Liu et al., TACL 2024）：当给 LLM 提供长输入时，开头和结尾的 token 获得不成比例的高注意力权重，中间区域信息被显著忽视。（来源: arXiv:2307.03172; [Lost-in-the-Middle问题深度分析](reference/06-检索策略与优化/03-Lost-in-the-Middle问题深度分析/)）
...
纯向量搜索对产品 SKU、错误码、专有术语等标识符本质上是噪声；纯关键词（BM25）又无法跨越词汇鸿沟。混合检索结合两者优势，是生产级 RAG 的**基线配置**。（来源: [混合检索RRF实战指南](reference/06-检索策略与优化/04-混合检索RRF实战指南/)）
...
其中 $\text{rank}_i(d)$ 是文档 $d$ 在第 $i$ 个检索系统结果中的排名（1-indexed），$k$ 通常设为 **60**（SIGIR 2009 原始论文经实验确定的鲁棒值）。RRF 基于相对排名而非原始分数，天然具有抗异常值、无需归一化的优势。（来源: [混合检索RRF实战指南](reference/06-检索策略与优化/04-混合检索RRF实战指南/)）
...
**Benchmark**：混合搜索 recall@5 提升 **8-12%**（vs 纯向量），RRF 准确率达 **88-94%**。（来源: NeuroLink; [混合检索RRF实战指南](reference/06-检索策略与优化/04-混合检索RRF实战指南/)）
